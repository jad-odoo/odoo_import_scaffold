...
def create_file_lib(file):
    ...
    with open(file, 'w') as f:
        ...
        f.write("    return val\n")
        f.write("\n\n")
        f.write("# This a template function that must be adapted to keep a real column\n")
        f.write("def keep_column_value(val):\n")
        f.write("    def keep_column_value_fun(line):\n")
        f.write("        if line['CSV_COLUMN'] != val:\n")
        f.write("            raise SkippingException(\"Column 'CSV_COLUMN' has not the right value %s\" % val)\n")
        f.write("        return line['CSV_COLUMN']\n")
